...
RRF 公式：`RRF_score(d) = Σ_i 1 / (k + rank_i(d))`，常数 k=60 用于平滑不同来源的排名差异。

**查询增强引擎**（亮点功能）

短查询（如"工资"、"张三"）直接用于向量检索召回率低。系统自动检测简单查询，使用便宜模型（qwen-turbo）并行执行三种增强：

- **查询改写（Rewrite）**：将口语化问题改写为适合知识库检索的正式表述
- **查询扩展（Expand）**：生成 3 个相关搜索关键词
- **HyDE**：生成假设性文档片段用于嵌入检索

增强后的多个查询变体并行执行向量检索，结果去重合并。对于复杂查询则跳过增强，直接检索以节省延迟。

**可配置人设**（亮点功能）

人设不再硬编码在代码中。系统通过 `persona_config` 表存储多个人设配置，支持通过前端页面动态创建、编辑和切换：

- 数据库中同时只能有一个人设处于激活状态
- 前端提供人设管理页面（AI 智能 → 人设管理），支持在线编辑大文本
- 代码中的 `ATRI_PERSONA` 常量仅作为数据库无激活人设时的 fallback

**防编造机制**

多层防护确保 AI 不捏造不存在的数据：

1. Prompt 层：逐字引用原则 — 引用知识库时必须逐字引用原文，禁止改写扩充
2. Few-Shot 层：正反示例 — 展示正确引用和错误编造的对比
3. 数据层：无数据时不注入上下文，LLM 收到"这轮没有命中系统数据"的指令

**12+ 业务域数据注入**

AI 直接查询数据库获取真实数据注入上下文，覆盖：系统概览、用户档案、考勤、请假、薪资、部门、职位、角色、权限、天气（高德 API）、AI 自身元数据等。每个域按用户权限过滤 — 只有拥有对应权限的角色才能触发对应数据查询。
...
上传 → 文本提取 → 清洗（去页眉页脚、过滤短行）→ 递归字符分块 (512 字符/块, 50 重叠)
    → text-embedding-v2 向量化 (每批 20 条) → pgvector 存储 → CAST 类型转换 + 余弦相似度检索 (top-5)
```

检索结果格式化为独立片段注入 LLM，每个片段用分隔线标明来源、分块序号和相关度，避免 LLM 将不同片段误认为连续文本。
...
- **融合检索**：向量检索 + 图谱查询结果通过 RRF 合并注入 LLM
...
├── app/
│   ├── ai/
│   │   ├── chat/            # 聊天核心：服务、分类器、Prompt、记忆、LLM 提供商、查询增强
│   │   ├── agent/           # Agent 工作流：LangGraph 状态机、节点、状态
│   │   ├── knowledge/       # 业务知识注入：12+ 域只读数据查询（按权限过滤）
│   │   ├── rag/             # RAG 管线：文档处理、分块、向量化、检索
│   │   ├── graph_rag/       # 知识图谱：NetworkX 图、RRF 融合检索
│   │   ├── multimodal/      # 多模态：Whisper 语音、TTS、视觉
│   │   └── observability/   # 可观测性：Token 计数、费用、Langfuse
│   ├── core/                # 核心：配置、认证、权限、数据库连接
│   ├── models/              # SQLAlchemy ORM 模型（含 PersonaConfig、LlmTrace）
│   ├── routers/             # FastAPI 路由（API 端点，含人设管理 CRUD）
